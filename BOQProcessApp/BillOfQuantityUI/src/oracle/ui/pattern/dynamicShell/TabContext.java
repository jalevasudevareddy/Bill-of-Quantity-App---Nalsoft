...
public final class TabContext implements Serializable {
     ...
     */
    public void setMainContent(String taskflowId) throws TabContentAreaDirtyException {
     ...
     * @throws TabContentAreaDirtyException If the tab state is currently dirty
     */
    public void setMainContent(String taskflowId, Map<String, Object> parameters) throws TabContentAreaDirtyException {
     ...
     */
    public void addOrSelectTab(String localizedName, String taskflowId) throws TabOverflowException {
     ...
     * @throws TabOverflowException If the maximum number of tabs is already shown
     */
    public void addTab(String localizedName, String taskflowId) throws TabOverflowException {
   ...
   */
    ...
    public TabContext() {
        if (_USE_SESSION_TRACKED_TABS) {
            String viewId = ControllerContext.getInstance()
                                             .getCurrentRootViewPort()
                                             .getViewId();
            String currentViewKey = String.format("__tc_%s", viewId);

            Tabs tabTracker = (Tabs) RequestContext.getCurrentInstance()
                                                   .getPageFlowScope()
                                                   .get(currentViewKey);
            if (tabTracker == null) {
                tabTracker = new Tabs();
            ...
            if (_tabTracker.getNumRendered() > 0) {
                ...
                do {
                    if (start == __MAX_TASKFLOWS) {
                        start = 0;
                        for (int i = __MAX_TASKFLOWS - 1; i >= 0; i--) {
    ...
    public void tabRemoveListener(ItemEvent itemEvent) {
        Integer tabIndex = (Integer) itemEvent.getComponent()
                                              .getAttributes()
                                              .get("tabIndex");
    ...
    public void closeCurrentTab(ActionEvent actionEvent) {
        Integer tabIndex = (Integer) actionEvent.getComponent()
                                                .getAttributes()
                                                .get("tabIndex");
    ...
    public void closeAllTabs(ActionEvent actionEvent) {
        ...
        for (int i = 0; i < tabs.size(); i++) {
            Tab tabToClose = tabs.get(i);
            if (tabToClose.isActive()) {
                this.removeTab(tabToClose.getIndex());
            }
        }
    }

    public void closeOtherTabs(ActionEvent actionEvent) {
        Integer tabIndex = (Integer) actionEvent.getComponent()
                                                .getAttributes()
                                                .get("tabIndex");
        List<Tab> tabs = this.getTabs();
        for (int i = 0; i < tabs.size(); i++) {
            Tab tabToClose = tabs.get(i);
            if (tabToClose.isActive() && tabToClose.getIndex() != tabIndex) {
                this.removeTab(tabToClose.getIndex());
            }
        }
    }
    ...
    static final TaskFlowId __BLANK =
        TaskFlowId.parse("/WEB-INF/oracle/ui/pattern/dynamicShell/infra/blank.xml#blank"); // NOTRANS
